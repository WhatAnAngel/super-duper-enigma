...
def calcular_gasto_energetico(peso, altura, sexo, edad, nivel_actividad, imc):
    if imc.startswith("Obesidad"):
        ...
        if sexo == 1:  # Masculino
            mb = 66.5 + (13.75 * peso) + (5.003 * altura) - (6.755 * edad)
        else:  # Femenino
            mb = 655.1 + (9.563 * peso) + (1.85 * altura) - (4.676 * edad)

    factores_actividad = [1.2, 1.375, 1.55, 1.725, 1.9]
    factor = factores_actividad[nivel_actividad]
    gasto_energetico_total = mb * factor

    return mb, gasto_energetico_total

def calcular_calorias(peso, altura, sexo, edad, nivel_actividad):
    imc = calcular_IMC(peso, altura)
    mb, gasto_energetico_total = calcular_gasto_energetico(peso, altura, sexo, edad, nivel_actividad, imc)
    ...
    return imc, mb, gasto_energetico_total, calorias

# Calcular calorías
imc, mb, gasto_energetico_total, calorias = calcular_calorias(peso, altura, sexo, edad, nivel_actividad)

# Mostrar resultados
print("Estado nutricional (IMC):", imc)
print("Gasto energético basal:", mb, "kcal")
